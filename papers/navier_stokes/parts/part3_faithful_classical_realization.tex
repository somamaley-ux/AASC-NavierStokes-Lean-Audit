...
\begin{manualdefinition}[18.0A (Realized continuation locus)]
\phantomsection\manualcurrentlabel{18.0A}\label{def:realized-continuation-locus}
For $x\in\mathcal I_{\mathrm{car}}(\T^3,\nu)$, the \emph{realized continuation locus} of $x$ is the fixed same-scope object consisting of the admitted carrier seed $x$, its maximal realized trajectory $\mathcal R_{\mathrm{NS}}(x)$, all smooth pre-horizon slice presentations, the restart / overlap identity relation supplied by local uniqueness, and the fixed horizon continuation question. The locus is a carrier-bound object; it is not a sequence of new admissibility decisions indexed by time.
\end{manualdefinition}

\begin{manualdefinition}[18.0B (Boundary trace of a realized lineage)]
\phantomsection\manualcurrentlabel{18.0B}\label{def:boundary-trace-realized-lineage}
A \emph{boundary trace} of a realized lineage is a lawful record of pre-horizon behavior approaching a proposed horizon: for example a norm profile, BKM profile, stretching profile, or sequence of smooth slice data. A boundary trace is not a new standing-bearing endpoint unless its standing-bearing content factors through the already fixed horizon discriminator.
\end{manualdefinition}

\begin{manualdefinition}[18.0C (Persistence regime)]
\phantomsection\manualcurrentlabel{18.0C}\label{def:persistence-regime-realized-lineage}
A realized lineage is a \emph{persistence regime} if its standing-bearing commitments persist under lawful redescription, smooth restart, overlap identification, and pre-horizon slice re-entry. Persistence here means fixed-domain continuation of the same standing-bearing object, not the generation of a new analytic continuation estimate.
\end{manualdefinition}

\begin{remark}[Claim-type discipline for the realized-lineage packet]\label{rem:partIII-claim-type-discipline}
The live burden of Part~III is not to produce the missing analytic bridge inequality or a new coercive continuation estimate. Its burden is the one required by the non-PDE-native impossibility proof class: once the official periodic classical scope and the realized lineage of Definition~\ref{def:realized-lineage-same-object-convention} are fixed, Part~III must show that finite-horizon failure induces a same-scope endpoint condition, that this condition has only the endpoint statuses later named in the realized-lineage trichotomy / collapse, and that no additional classical-layer selector or continuation gate is introduced on that route.
\end{remark}
...
primitive theorem into the official PDE formulation using only the local analytic packet proved in
...
Lemma~\ref{lem:strong-completeness-to-realized-completeness} records how the broader continuation-completeness package would descend to the realized local class if one chooses to read the manuscript through that wider architectural lens. On that same narrow horizon-detectability route, this lemma is explanatory rather than load-bearing for the official periodic theorem: the official periodic theorem uses the horizon-detectability chain through \HorizonDetectabilityRef{} instead of the broader strong-completeness bridge. It is therefore retained explicitly as broader conceptual architecture, not as a necessary ingredient in the official-theorem spine.
\end{remark}
...
No external corpus theorem is added at this stage: the bridge is constructed inside the paper using only the manuscript-local analytic packet proved in Appendix~B and recorded later in Proposition~\ref{prop:three-level-support-ledger}.
...
\begin{manuallemma}[19E.1 (Continuation-locus fixity)]
\phantomsection\manualcurrentlabel{19E.1}\label{lem:continuation-locus-fixity}
For a standing-positive admitted carrier state $x$, smooth restart and overlap uniqueness preserve the realized continuation locus of Definition~\ref{def:realized-continuation-locus}. In particular, a restart from any smooth time $t_0<T_*(x)$ is a presentation of the same locus and not a same-scope continuation object.
\end{manuallemma}

\begin{proof}
Theorem~\ref{thm:restart-consistency-same-object-persistence} shows that restarting from a smooth slice produces a local classical realization that agrees with the original trajectory on overlap. Corollary~\ref{cor:pre-horizon-slice-reentry} then shows that the slice re-enters the same carrier and remains standing-positive. These two facts preserve every component of Definition~\ref{def:realized-continuation-locus}: the admitted seed, the realized trajectory up to lawful presentation, the restart / overlap identity relation, and the fixed continuation question. Hence the locus is fixed.
\end{proof}

\begin{manuallemma}[19E.2 (Boundary-trace nonpromotion)]
\phantomsection\manualcurrentlabel{19E.2}\label{lem:boundary-trace-nonpromotion}
A boundary trace in the sense of Definition~\ref{def:boundary-trace-realized-lineage} cannot become an independent same-scope endpoint gate. If it is used in the live route, it is either licensed bookkeeping, quotient-visible witness bookkeeping subordinate to the horizon discriminator, or an illicit metric / representative gate.
\end{manuallemma}

\begin{proof}
A boundary trace is built from pre-horizon lawful presentations of one realized lineage. By Corollary~\ref{cor:skin-non-authority-representative-diagnostics}, representative-only data do not carry standing authority. By Theorem~\ref{thm:metric-non-authority-periodic-carrier}, norm profiles, BKM profiles, stretching profiles, and other metric trends are bookkeeping unless subordinate to an already fixed witness; if promoted independently they are illicit metric gates. Thus a trace has no endpoint authority apart from the fixed horizon discriminator.
\end{proof}

\begin{manuallemma}[19E.3 (Persistence-regime realization)]
\phantomsection\manualcurrentlabel{19E.3}\label{lem:persistence-regime-realization}
Let $x\in\mathcal I_{\mathrm{car}}(\T^3,\nu)$ be standing-positive. Then its maximal realized periodic lineage is a persistence regime in the sense of Definition~\ref{def:persistence-regime-realized-lineage} on $[0,T_*(x))$.
\end{manuallemma}

\begin{proof}
Lawful redescription persistence follows from Lemma~\ref{lem:redescription-commutes-with-realization} and Lemma~\ref{lem:continuation-skin-invariant}. Smooth restart and overlap persistence follow from Theorem~\ref{thm:restart-consistency-same-object-persistence}. Pre-horizon standing re-entry follows from Corollary~\ref{cor:pre-horizon-slice-reentry}. The same continuation question is fixed by Proposition~\ref{prop:partIII-same-continuation-notion}. These are exactly the persistence clauses of Definition~\ref{def:persistence-regime-realized-lineage}.
\end{proof}
...
\paragraph{Burden status.}
The official local classical class has already been realized. What remains here is to show that realization is faithful, that no same-scope continuation criterion appears at the classical layer, and that any hypothetical finite-time breakdown is already carrier-visible on the fixed torus carrier.
...
\begin{proof}
Corollary~\ref{cor:every-official-datum-admitted} sends every official datum into a unique standing-positive admitted carrier state, and Theorem~\ref{thm:periodic-carrier-unique-interior} identifies that state with the unique admissible interior on the fixed torus carrier. Definition~\ref{def:realized-lineage-same-object-convention} and Definition~\ref{def:realized-continuation-locus} then fix the realized maximal classical object as one same-scope continuation locus, while Proposition~\ref{prop:partIII-same-continuation-notion} states theorem-locally that the same continuation question used in Part~III is exactly the one fixed in Part~II.

For smooth interior times, Theorem~\ref{thm:restart-consistency-same-object-persistence} shows that restarting from $u_x(t_0)$ reproduces the same realized lineage on overlap rather than creating a second same-scope object, Corollary~\ref{cor:pre-horizon-slice-reentry} shows that every such smooth slice re-enters the same admitted carrier and is standing-positive there, and Lemma~\ref{lem:continuation-locus-fixity} packages these facts as continuation-locus fixity. Proposition~\ref{prop:periodic-carrier-interface-quotient-history} together with Corollary~\ref{cor:no-path-dependent-periodic-admissibility} then rules out treating later restart representatives or later lawful presentations as fresh admissibility decisions inside the same fixed scope. Therefore the maximal realized periodic evolution issued from $x$ is one standing-bearing same-scope determination on the fixed carrier, not a stack of independently reclassified admissibility states indexed by time.
\end{proof}
...
The standing/intelligibility packet ST1--ST5, the PDE-native admissibility packet, the quotient / history / ATS packet, and the carrier-to-lineage / no-dynamics packet now allow the manuscript to say something sharper than the earlier two-branch rhetoric. On a realized periodic lineage there are only three theorem-locally typable ways to keep a finite-horizon endpoint claim alive while still pretending to remain inside the fixed same scope: as a standing-positive in-scope witness, as terminal standing failure, or as an illicit same-scope gate / scope move. The next packet makes that trichotomy explicit and then collapses the non-witness branches before witness exhaustion is invoked in Section~\ref{sec:singular-images-official-theorem}.


\subsubsection*{Horizon bridge object and local finite-horizon reductio}
\addcontentsline{toc}{subsubsection}{Horizon bridge object and local finite-horizon reductio}
The endpoint closeout can now be read locally. A finite-horizon assumption is opened as the exact complement of global smooth continuation for one fixed realized lineage. The bridge object below packages the data forced by that assumption; it does not add a continuation estimate and it does not contain global smoothness by definition.

\begin{manualdefinition}[20B.4 (Horizon bridge object)]
\phantomsection\manualcurrentlabel{20B.4}\label{def:ns-horizon-bridge-object}
Let $x\in\mathcal I_{\mathrm{car}}(\T^3,\nu)$ be standing-positive and let
$\mathcal R_{\mathrm{NS}}(x)=(u_x,p_x)$ be its realized lineage. For a proposed finite horizon $T=T_*(x)<\infty$, the \emph{Navier--Stokes horizon bridge object}
\[
B^{NS}_{\mathrm{hor}}(x,T)
\]
records the following same-scope data:
\begin{enumerate}[label=\textbf{B\arabic*.}, leftmargin=3.2em, itemsep=0.15em]
\item the official datum $u_0\in\mathcal D_{\mathrm{off}}(\T^3,\nu)$ determined by $x$;
\item the fixed periodic carrier: $\T^3$, $\nu>0$, zero forcing, smooth divergence-free class, and the fixed mean-zero pressure gauge;
\item the same smooth periodic continuation question fixed in Part~II and realized in Part~III;
\item the admitted carrier seed $x=[\Sigma(u_0)]$ as the source of the realized lineage;
\item the identity of $\mathcal R_{\mathrm{NS}}(x)$ as one same-scope maximal classical lineage, not a sequence of independently reclassified time slices;
\item the pre-horizon witness packet: every $t<T$ gives a smooth slice that re-enters the official carrier as a lawful presentation of the same realized lineage;
\item the carrier-side horizon discriminator $\mathfrak D_T(x)=0$ supplied by finite maximality at the first non-extendable horizon;
\item metric-witness subordination: BKM, critical-norm, vorticity, strain, frequency, and metric-divergence reports attached to the horizon are only quotient-visible witness bookkeeping unless promoted to an illicit gate;
\item the branch data $(\beta(x),\mathfrak B(x))$, where $\beta(x)$ is the branch tag and $\mathfrak B(x)\subseteq\{S1,S2,S3,S4,S5\}$ is the induced branch-forcing set; and
\item no independent endpoint authority: the bridge object introduces no new PDE estimate, continuation criterion, hidden selector, or second same-scope endpoint gate.
\end{enumerate}
\end{manualdefinition}

\begin{manualproposition}[20B.5 (Horizon bridge object is non-circular)]
\phantomsection\manualcurrentlabel{20B.5}\label{prop:ns-horizon-bridge-noncircular}
The object $B^{NS}_{\mathrm{hor}}(x,T)$ does not contain the conclusion $T_*(x)=\infty$ by definition. It packages the finite-horizon countercase data forced by the fixed carrier, realized-lineage identity, horizon discriminator, metric-bookkeeping discipline, and branch extraction.
\end{manualproposition}

\begin{proof}
Clauses B1--B6 fix the official datum, carrier, continuation question, carrier seed, realized-lineage identity, and pre-horizon witness availability. Clause B7 records the finite-horizon discriminator produced by the local assumption $T_*(x)<\infty$. Clauses B8--B10 assign the role of metric diagnostics and branch data without adding endpoint authority. None of these clauses asserts global continuation. The conclusion $T_*(x)=\infty$ is obtained only after the trichotomy, collapse of the non-witness branches, and elimination of the remaining standing-positive witness branch.
\end{proof}

\begin{manualtheorem}[20B.6 (Horizon bridge object induces the endpoint-status trichotomy)]
\phantomsection\manualcurrentlabel{20B.6}\label{thm:ns-horizon-bridge-trichotomy}
If $B^{NS}_{\mathrm{hor}}(x,T_*(x))$ is in force, then the associated finite-horizon endpoint condition has exactly one of the three statuses later stated in Theorem~\ref{thm:same-scope-continuation-failure-dichotomy}: standing-positive admissible in-scope witness, terminal standing failure, or illicit same-scope gate / scope move.
\end{manualtheorem}

\begin{proof}
The bridge object fixes the same carrier and realized lineage, records the horizon discriminator, and keeps metric diagnostics subordinate to the fixed witness packet unless they are promoted to an illicit gate. These are precisely the inputs used by Theorem~\ref{thm:same-scope-continuation-failure-dichotomy}. The object therefore enters that trichotomy without adding another endpoint status.
\end{proof}

\begin{manualdefinition}[20B.7 (Exact-complement finite-horizon annotation)]
\phantomsection\manualcurrentlabel{20B.7}\label{def:ns-local-finite-horizon-annotation}
The notation
\[
[T_*(x)<\infty]_i
\]
denotes the proof-act annotation generated when the sole local assumption $T_*(x)<\infty$ is opened as the exact-complement countercase against global smooth continuation of the fixed realized lineage $\mathcal R_{\mathrm{NS}}(x)$. The annotation records proof role only; it is not an additional object-level premise.
\end{manualdefinition}

\begin{manualtheorem}[20B.8 (Annotation is not a premise)]
\phantomsection\manualcurrentlabel{20B.8}\label{thm:ns-annotation-not-premise}
The exact-complement annotation does not transform the discharged assumption into
\[
T_*(x)<\infty\ \wedge\ \mathrm{ReductioCountercase}_{NS}(T_*(x)<\infty;T_*(x)=\infty).
\]
The only object-level assumption opened and later discharged is $T_*(x)<\infty$.
\end{manualtheorem}

\begin{proof}
The bracket $[\,\cdot\,]_i$ marks the assumption's local proof role: it is the branch whose survival blocks the endpoint for the fixed realized lineage. The proof does not add a second mathematical assertion to the assumption. All later bridge, trichotomy, and branch-extraction roles are consequences of using that sole finite-horizon assumption as the local exact complement inside the fixed same-scope endpoint proof.
\end{proof}

\begin{manualtheorem}[20B.9 (Local finite-horizon countercase induces the horizon bridge)]
\phantomsection\manualcurrentlabel{20B.9}\label{thm:ns-countercase-induces-horizon-bridge}
For a standing-positive admitted carrier state $x$,
\[
[T_*(x)<\infty]_i\quad\Longrightarrow\quad B^{NS}_{\mathrm{hor}}(x,T_*(x)).
\]
\end{manualtheorem}

\begin{proof}
The assumption $T_*(x)<\infty$ supplies the proposed finite horizon. The fixed datum and carrier clauses are supplied by Corollary~\ref{cor:every-official-datum-admitted} and Theorem~\ref{thm:periodic-carrier-unique-interior}. Realized-lineage identity and pre-horizon re-entry are supplied by Theorem~\ref{thm:carrier-to-lineage-binding}, Theorem~\ref{thm:restart-consistency-same-object-persistence}, and Corollary~\ref{cor:pre-horizon-slice-reentry}. The carrier-side horizon discriminator is supplied by Theorem~\ref{thm:horizon-detectability}. Metric-witness subordination is supplied by Propositions~\ref{prop:metric-divergence-skin-non-authority} and \ref{prop:quotient-visible-witness-vs-illicit-metric-gate}. The branch tag and branch-forcing set are supplied by Lemma~\ref{lem:breakdown-branch-assignment} and Definition~\ref{def:branch-forcing-set}. Thus all clauses of Definition~\ref{def:ns-horizon-bridge-object} are forced by the local finite-horizon countercase.
\end{proof}

\begin{manualtheorem}[20B.10 (Local finite-horizon countercase contradiction)]
\phantomsection\manualcurrentlabel{20B.10}\label{thm:ns-local-finite-horizon-contradiction}
For every standing-positive admitted carrier state $x$,
\[
[T_*(x)<\infty]_i\quad\Longrightarrow\quad\bot.
\]
\end{manualtheorem}

\begin{proof}
By Theorem~\ref{thm:ns-countercase-induces-horizon-bridge}, the local finite-horizon countercase induces $B^{NS}_{\mathrm{hor}}(x,T_*(x))$. By Theorem~\ref{thm:ns-horizon-bridge-trichotomy}, the associated endpoint condition enters the realized-lineage trichotomy of Theorem~\ref{thm:same-scope-continuation-failure-dichotomy}. The terminal standing-failure branch is excluded by Proposition~\ref{prop:terminal-standing-failure-no-endpoint}; the illicit gate / scope-move branch is not a licit same-scope endpoint status by Corollary~\ref{cor:no-third-same-scope-endpoint-status}. The remaining standing-positive witness branch is exhausted by Proposition~\ref{prop:same-scope-witness-exhaustion} and converted into a typed singular candidate by Lemma~\ref{lem:singular-image-induces-typed-candidate}. Propositions~\ref{prop:no-illicit-singular-image} and \ref{prop:no-quotient-visible-singular-image}, together with Corollary~\ref{cor:no-third-branch}, eliminate the resulting branches. Hence the local finite-horizon assumption yields contradiction.
\end{proof}

\begin{manualtheorem}[20B.11 (Annotation discharge gives global smoothness for the lineage)]
\phantomsection\manualcurrentlabel{20B.11}\label{thm:ns-annotation-discharge-global-smoothness}
For every standing-positive admitted carrier state $x$, the local contradiction from $[T_*(x)<\infty]_i$ discharges the original finite-horizon assumption itself. Consequently $T_*(x)=\infty$.
\end{manualtheorem}

\begin{proof}
By Theorem~\ref{thm:ns-annotation-not-premise}, the only object-level assumption in the local subproof is $T_*(x)<\infty$. Theorem~\ref{thm:ns-local-finite-horizon-contradiction} gives a contradiction from that assumption, so reductio yields $\neg(T_*(x)<\infty)$. Since Theorem~\ref{thm:local-classical-realization} gives a maximal classical interval $[0,T_*(x))$ with $T_*(x)>0$ and the only alternative to global time is finite maximal time, the result is $T_*(x)=\infty$.
\end{proof}


\begin{manualdefinition}[20C.0 (Finite-horizon endpoint condition)]
\phantomsection\manualcurrentlabel{20C.0}\label{def:finite-horizon-endpoint-condition}
Let $x\in\mathcal I_{\mathrm{car}}(\T^3,\nu)$ and let $\mathcal R_{\mathrm{NS}}(x)=(u_x,p_x)$ be its maximal realized lineage on $[0,T_*(x))$. If $T_*(x)<\infty$, the \emph{finite-horizon endpoint condition} is the same-scope continuation condition determined by failure of smooth extension beyond $T_*(x)$ on the fixed periodic carrier. It is not optional descriptive surplus: it is the theorem-bearing obstruction that a counterexample to Fefferman~(B) would have to supply.
\end{manualdefinition}

\begin{manualtheorem}[20C.0A (Finite-horizon failure induces a same-scope endpoint condition)]
\phantomsection\manualcurrentlabel{20C.0A}\label{thm:finite-horizon-failure-induces-endpoint-condition}
If $x\in\mathcal I_{\mathrm{car}}(\T^3,\nu)$ has $T_*(x)<\infty$, then finite maximality of $\mathcal R_{\mathrm{NS}}(x)$ induces the finite-horizon endpoint condition of Definition~\ref{def:finite-horizon-endpoint-condition}. This condition belongs to the fixed-domain comparison class unless one of the declared periodic commitments is altered, in which case the alleged failure is not a same-scope counterexample to the official periodic theorem.
\end{manualtheorem}

\begin{proof}
By Theorem~\ref{thm:local-classical-realization}, $T_*(x)$ is the maximal classical realization time on the fixed torus carrier. If $T_*(x)<\infty$, maximality says exactly that no smooth continuation on the same carrier, with the same viscosity, forcing status, pressure gauge, datum anchor, and continuation notion, is available past $T_*(x)$. That is the continuation condition named in Definition~\ref{def:finite-horizon-endpoint-condition}. If a later description changes any of those commitments, Definition~\ref{def:fixed-domain-comparison-class} classifies it outside the fixed-domain comparison class. Otherwise the condition is same-scope and is governed by the endpoint-status trichotomy below.
\end{proof}
...
Let $x\in\mathcal I_{\mathrm{car}}(\T^3,\nu)$ and let $\mathcal R_{\mathrm{NS}}(x)=(u_x,p_x)$ have maximal interval $[0,T_*(x))$ with $T_*(x)<\infty$. Then the finite-horizon endpoint condition induced by Definition~\ref{def:finite-horizon-endpoint-condition} has exactly one of the following theorem-local statuses (equivalently, any attempted same-scope description of that condition must fall into one of these statuses):
\begin{enumerate}[label=(\roman*), leftmargin=2.2em]
...
Assume instead that the finite-horizon endpoint condition is not presented through a standing-positive admissible in-scope witness. Because Theorem~\ref{thm:carrier-to-lineage-binding} fixes the realized evolution as one standing-bearing same-scope determination and Theorem~\ref{thm:no-admissibility-dynamics-realized-lineages} rules out time-indexed admissibility change on that determination, the endpoint condition cannot survive as a fresh stage-indexed status. There are then only two remaining ways to keep the endpoint claim alive while still pretending to stay in the same scope.

First, the condition may be described as completing the same realized lineage only by leaving admissible representability while preserving the same torus carrier and lineage identity. Proposition~\ref{prop:exterior-endpoint-standing-failure} identifies that case exactly with terminal standing failure, giving clause~(ii).

Second, the condition may be kept alive only by smuggling in a same-scope discriminator. That is exactly what the factorization, history, role, metric, and classical-layer packets now forbid theorem-locally: Theorem~\ref{thm:factorization-discipline-same-scope} rules out standing-relevant functionals that fail to factor through the fixed quotient, Theorem~\ref{thm:history-factorization-no-path-dependent-admissibility} together with Corollary~\ref{cor:no-path-dependent-periodic-admissibility} rules out path-, order-, or history-representative-dependent admissibility, Corollary~\ref{cor:skin-non-authority-representative-diagnostics} rules out representative-only skin as standing authority, Proposition~\ref{prop:metric-divergence-skin-non-authority} rules out raw metric divergence as an independent same-scope gate, Proposition~\ref{prop:quotient-visible-witness-vs-illicit-metric-gate} rules out a third metric-gate status, and Theorem~\ref{thm:no-independent-classical-discriminator} rules out any surviving classical-layer continuation discriminator. Any such use is therefore clause~(iii): an illicit same-scope gate / scope move rather than a licit same-scope endpoint status.

The three clauses are exhaustive for finite-horizon endpoint conditions because carrier-to-lineage binding and no-admissibility-dynamics remove time-indexed reclassification, Theorem~\ref{thm:st3-standing-admissibility-equivalence} together with Corollary~\ref{cor:no-standing-neutral-status-fixed-periodic} remove any neutral same-scope middle status, and the factorization / role / metric packet removes any fourth same-scope standing-relevant proposal form. Finally, Proposition~\ref{prop:terminal-standing-failure-no-endpoint} excludes clause~(ii) as a licit same-scope completion, while clause~(iii) is already non-licit by the just-cited factorization, role, metric, and classical-layer theorems. Hence only clause~(i) remains as a licit same-scope endpoint status.
\end{proof}
...
\begin{manualtheorem}[20C.4 (Counterexample soundness for fixed same-scope endpoints)]
\phantomsection\manualcurrentlabel{20C.4}\label{thm:counterexample-soundness-fixed-same-scope}
In a non-degenerate same-scope theorem-bearing proof/refutation regime, a counterexample to a universal continuation theorem must supply a licit same-scope endpoint condition contradicting the asserted continuation. Applied to Fefferman's official periodic Navier--Stokes statement~(B), this requirement is not an extra AASC filter added to the official problem and does not redefine Fefferman counterexamplehood: it states the role conditions already required for an ambient mathematical object to function as a counterexample to Fefferman~(B). Theorem~\ref{thm:ambient-proof-system-subsumption} and Theorem~\ref{thm:fefferman-counterexample-subsumption} make that role-equivalence explicit. Therefore, if every finite-horizon endpoint condition is classified as terminal standing failure, an illicit same-scope gate / scope move, or a standing-positive witness branch eliminated by Section~\ref{sec:singular-images-official-theorem}, then no same-scope counterexample to Fefferman~(B) exists.
\end{manualtheorem}

\begin{proof}
We separate ordinary universal-theorem logic, ambient proof-system standing, and fixed-scope endpoint status.
\begin{enumerate}[leftmargin=2.2em,itemsep=0.2em]
\item Fefferman~(B), in the periodic form restated in Theorem~\ref{thm:official-periodic-pde-theorem}, is universal over the fixed official datum class of Definition~\ref{def:official-periodic-data}. A counterexample must therefore start from an official datum and preserve the same equations, carrier, viscosity, forcing status, gauge, and smooth periodic continuation question.
\item A counterexample in ordinary mathematics is not merely the object-level phrase $T_*<\infty$. It is an ambient proof/refutation construction showing that an object satisfying the official hypotheses also satisfies the negation of the asserted conclusion. By Lemma~\ref{lem:counterexamplehood-theorem-bearing-construction}, such counterexamplehood requires admissible object formation, proof-standing, fixed reference, same-target fidelity, and final theorem consequence.
\item The appeal to ZFC, classical analysis, or any other ambient proof system does not bypass those roles. By Theorem~\ref{thm:ambient-proof-system-subsumption} and Corollary~\ref{cor:no-ambient-proof-system-escape}, any ambient proof system capable of establishing a theorem-bearing counterexample instantiates the fixed-scope kernel roles at the level of proof/refutation use.
\item If a purported counterexample changes any official commitment, it is outside the fixed-domain comparison class of Definition~\ref{def:fixed-domain-comparison-class}; it may describe a different problem, but it is not a counterexample to the theorem stated here.
\item If a purported counterexample preserves those commitments, then the only way it can contradict the universal global-continuation conclusion is by finite maximality on the same realized lineage: $T_*(x)<\infty$ for the carrier state associated to the datum. By Theorem~\ref{thm:finite-horizon-failure-induces-endpoint-condition}, that finite maximality induces a finite-horizon endpoint condition for the same carrier, same lineage, and same continuation question.
\item Because the official periodic proof/refutation regime is non-degenerate by Theorem~\ref{thm:official-periodic-ns-nondegenerate}, because ambient Fefferman counterexamplehood is subsumed by the kernel by Theorem~\ref{thm:fefferman-counterexample-subsumption}, and because the fixed-scope kernel applies by Corollary~\ref{cor:kernel-applies-official-periodic-necessity}, a theorem-bearing same-scope counterexample must complete through a licit same-scope endpoint status. A standing-negative object, terminal standing failure, hidden selector, metric-only gate, or undeclared scope move cannot refute the universal theorem while also preserving the declared comparison class.
\item Theorem~\ref{thm:same-scope-continuation-failure-dichotomy} exhausts the endpoint-status options for that endpoint condition. Proposition~\ref{prop:terminal-standing-failure-no-endpoint} and Corollary~\ref{cor:no-third-same-scope-endpoint-status} collapse the non-witness branches, and Section~\ref{sec:singular-images-official-theorem} eliminates the remaining standing-positive witness branch.
\end{enumerate}
Thus a same-scope counterexample would have to supply a licit same-scope endpoint condition, but no such condition survives. Therefore no same-scope counterexample to Fefferman~(B) exists.
\end{proof}
...
The abstract branch-forcing result now applies directly on the live carrier-certified route: by
Lemma~\ref{lem:part1-to-singular}, together with the carrier-certified Structural Exhaustion packet of
Theorem~\ref{thm:ns-carrier-verifies-rule-package} and
Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}, any attempt to treat a classical
blow-up episode as standing-positive under the fixed carrier commitments and in the absence of a
licensed critical bridge must instantiate at least one of $(S1)$--$(S5)$. Applied to $\mathfrak W(x)$,
this yields only diagnostic content: the breakdown witness forces a nonempty branch set of forbidden
same-scope moves that witness how the impossible escape attempt would have to appear.
...
\begin{proof}
Suppose $x\in\mathcal I_{\mathrm{car}}(\T^3,\nu)$ had a classical singular image. By Theorem~\ref{thm:finite-horizon-failure-induces-endpoint-condition}, this finite maximality induces a same-scope finite-horizon endpoint condition on the fixed periodic carrier. Theorem~\ref{thm:same-scope-continuation-failure-dichotomy} then forces that endpoint condition into the realized-lineage status trichotomy of clauses~(i)--(iii).
...
\item Proposition~\ref{prop:same-scope-witness-exhaustion} and Lemma~\ref{lem:singular-image-induces-typed-candidate}, with Theorem~\ref{thm:constructive-branch-forcing} grounded in the carrier-certified Structural Exhaustion packet of Theorem~\ref{thm:ns-carrier-verifies-rule-package} and Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}, convert the entire standing-positive continuation content of the remaining licit branch into an explicit typed singular candidate extractor $\mathfrak X(x)$ carrying a nonempty forced branch set $\mathfrak B(x)\subseteq\{S1,S2,S3,S4,S5\}$.
