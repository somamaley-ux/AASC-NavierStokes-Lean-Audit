...
Fourth, only after that finite classification and elimination packet is in the theorem-local chain do zero-parameter certification together with the narrowed continuation-completeness package verify the standing-positive carrier-side completeness claim actually used on the live proof chain, and Theorem~\ref{thm:ns-carrier-verifies-rule-package} verifies the Part~II rule packet for this specific periodic carrier in that theorem-local sense. Subsection~13.2A records the explicit translation dictionary used on that standing-positive route, and Section~13.2D interprets the derived carrier-specific exhaustion theorem as a same-scope existence statement on the fixed carrier. The completeness claims in these subsections are therefore frozen at the domain they visibly prove rather than silently enlarged to full same-scope totality before the later endpoint-status audit.
...
\begin{manualdefinition}[3.4A (Fixed-domain comparison class for the official periodic problem)]
\phantomsection\manualcurrentlabel{3.4A}\label{def:fixed-domain-comparison-class}
For the official periodic Navier--Stokes target, the \emph{fixed-domain comparison class} is the class of proposals that preserve simultaneously the torus carrier $\T^3$, the fixed viscosity $\nu>0$, zero forcing, the smooth divergence-free periodic datum class, the fixed mean-zero pressure gauge, the lawful redescription class of Definition~\ref{def:standard-carrier}, and the same-scope continuation question fixed in Lemma~\ref{lem:bridge-fixed-periodic-scope}. A proposal that adds an independent selector, tolerance, threshold, horizon rule, or admissibility gate is not in this comparison class unless the added object is already quotient-visible bookkeeping in the sense of the later carrier packet.
\end{manualdefinition}

\begin{manualdefinition}[3.4B (Same-scope counterexample)]
\phantomsection\manualcurrentlabel{3.4B}\label{def:same-scope-counterexample-fixed-domain}
A \emph{same-scope counterexample} to the official periodic endpoint is a purported finite-horizon failure proposal that belongs to the fixed-domain comparison class of Definition~\ref{def:fixed-domain-comparison-class} and nevertheless claims to change the endpoint status of the realized periodic lineage. A proposed failure that changes one of the fixed-domain commitments is a different-domain proposal rather than a same-scope counterexample to the theorem proved here.
\end{manualdefinition}


\begin{manualdefinition}[3.4C (Non-degenerate theorem-bearing regime)]
\phantomsection\manualcurrentlabel{3.4C}\label{def:nondegenerate-theorem-bearing-regime}
A \emph{non-degenerate theorem-bearing regime} is a fixed-scope reasoning regime in which all of the following are present: determinate reference, a fixed comparison domain, standing-bearing consequence claims, lawful redescription identity, irreversible commitment, fail-closed admissibility, nontrivial admitted content, and no arbitrary post-hoc repair of failed standing. This definition is intentionally method-neutral: it applies to a PDE regime, algebraic regime, formalized kernel, or any other mathematical setting once it is used to make determinate theorem-bearing same-scope claims.
\end{manualdefinition}

\begin{manualdefinition}[3.4D (Kernel instantiation)]
\phantomsection\manualcurrentlabel{3.4D}\label{def:kernel-instantiation}
A theorem-bearing regime \emph{instantiates the fixed-scope kernel} when its admissibility, standing, reference preservation, irreversibility, redescription invariance, and fixed-scope exhaustion govern which consequence claims remain licit at that scope. Kernel instantiation is not the adoption of an optional vocabulary; it is the presence of the minimal governance structure required for non-degenerate same-scope consequence.
\end{manualdefinition}

\begin{manualtheorem}[3.K (Kernel Necessity for non-degenerate regimes)]
\phantomsection\manualcurrentlabel{3.K}\label{thm:kernel-necessity-nondegenerate-regimes}
Every non-degenerate theorem-bearing regime in the sense of Definition~\ref{def:nondegenerate-theorem-bearing-regime} instantiates the fixed-scope kernel in the sense of Definition~\ref{def:kernel-instantiation}. Consequently, kernel governance is not an optional modeling convention, interpretive preference, or external formalism imposed on such a regime; it is the minimal fixed-scope structure required for determinate standing-bearing consequence.
\end{manualtheorem}

\begin{proof}
A non-degenerate theorem-bearing regime already has a fixed comparison domain, determinate reference, standing-bearing consequence, lawful redescription identity, irreversible commitment, fail-closed admissibility, and nontrivial admitted content. These are exactly the handles through which same-scope claims can remain determinate while preserving identity and excluding post-hoc repair. If admissibility did not govern standing, the regime would have no theorem-bearing licit / illicit distinction. If reference or lawful redescription were not preserved, the regime would not have same-scope identity. If irreversibility or fail-closed enforcement failed, rejected consequence could be repaired by later reclassification. If fixed-scope exhaustion failed, an additional selector could decide the same claim while pretending not to change scope. Thus the regime necessarily carries the fixed-scope kernel structure. No optional adoption step is involved.
\end{proof}


\begin{tcolorbox}[breakable, title={Minimality note: these are theorem-bearing conditions, not extra PDE hypotheses}, colback=yellow!2!white, colframe=yellow!45!black]
The non-degeneracy clauses in Definition~\ref{def:nondegenerate-theorem-bearing-regime} are not additional physical assumptions, PDE regularity hypotheses, or optional AASC decorations. They are the minimum conditions under which a mathematical regime has a determinate subject matter: fixed reference, a fixed comparison domain, consequence-bearing assertions, identity under lawful redescription, irreversible commitment to the declared problem, fail-closed treatment of out-of-domain moves, and at least one admitted case. If a critic denies those clauses, the critic is not merely rejecting a formalism; the critic is denying that the regime supports a stable theorem to prove or refute at the declared scope. Thus the kernel is not smuggled into the problem as a preferred vocabulary. It is the fixed-scope consequence of treating the problem as a non-degenerate theorem-bearing object.
\end{tcolorbox}

\begin{manualcorollary}[3.K1 (No framework opt-out under non-degeneracy)]
\phantomsection\manualcurrentlabel{3.K1}\label{cor:no-framework-optout-nondegenerate}
A reader cannot reject the fixed-scope kernel while preserving non-degenerate same-scope theorem-bearing discourse. To opt out of the kernel one must deny at least one of the non-degeneracy clauses of Definition~\ref{def:nondegenerate-theorem-bearing-regime}.
\end{manualcorollary}

\begin{proof}
This is the contrapositive of Theorem~\ref{thm:kernel-necessity-nondegenerate-regimes}. If all non-degeneracy clauses are retained, kernel instantiation follows. If kernel instantiation is denied, at least one clause needed for determinate same-scope theorem-bearing consequence has been abandoned.
\end{proof}

\subsection*{Ambient counterexamplehood and kernel-role subsumption}
\addcontentsline{toc}{subsection}{Ambient counterexamplehood and kernel-role subsumption}
\label{subsec:ambient-counterexamplehood-kernel-subsumption}

\begin{tcolorbox}[breakable, title={Jurisdiction note: ambient proof systems do not escape the kernel}, colback=cyan!2!white, colframe=cyan!55!black]
The fixed-scope kernel is not a competing syntax for ZFC or classical analysis. It is a role theorem about what any non-degenerate proof/refutation regime must already do in order for an object to count as a theorem-bearing proof or counterexample. If an ambient proof system supplies admissible object formation, proof-standing, fixed reference to the target theorem, same-target fidelity, and final theorem consequence, then it performs the kernel roles whether or not it uses AASC vocabulary. If it does not supply those roles, it cannot support determinate theorem-bearing counterexamplehood.
\end{tcolorbox}

\begin{manualdefinition}[3.K2 (Ambient proof/refutation regime)]
\phantomsection\manualcurrentlabel{3.K2}\label{def:ambient-proof-refutation-regime}
An \emph{ambient proof/refutation regime} for a fixed universal mathematical theorem is the ordinary mathematical setting in which a proposed proof or counterexample is formed, checked, and allowed to bear consequence for that theorem. The regime may be ZFC set theory, classical analysis, a formal proof assistant, or any other accepted proof environment. It is non-degenerate for the theorem only if it fixes the target statement, identifies admissible objects, preserves reference to the target, distinguishes theorem-bearing from non-theorem-bearing assertions, and gives final consequence to a successful proof or counterexample.
\end{manualdefinition}

\begin{manualtheorem}[3.K3 (Ambient proof-system\\subsumption)]
\phantomsection\manualcurrentlabel{3.K3}\label{thm:ambient-proof-system-subsumption}
Every non-degenerate ambient proof/refutation regime for a fixed universal theorem instantiates the fixed-scope kernel at the level of theorem-bearing use. In particular, ordinary ZFC / classical-analysis counterexamplehood supplies admissibility, standing, reference, same-target fidelity, and final consequence only by performing the work of the kernel roles.
\end{manualtheorem}

\begin{proof}
Let an ambient proof/refutation regime be able to establish a counterexample to a fixed universal theorem. Then it must determine which objects are admissibly formed for the theorem's domain; otherwise the alleged counterexample has no target-licensed object. It must distinguish an assertion with proof-standing from a mere string or informal picture; otherwise no theorem-bearing consequence follows. It must preserve reference to the fixed theorem and its hypotheses; otherwise the alleged counterexample may concern a different target. It must make the refutation final at the act of proof, rather than allowing later repair by changing the theorem, domain, or equivalence class. These are exactly admissibility, standing, reference, same-regime fidelity, and irreversibility/finality at the level of theorem-bearing use. By Theorem~\ref{thm:kernel-necessity-nondegenerate-regimes}, such a regime instantiates the fixed-scope kernel. Renaming these roles as ``ordinary ZFC admissibility'' or ``classical-analysis verification'' does not remove them; it is governance-equivalent performance of the same work.
\end{proof}


\begin{tcolorbox}[breakable, title={Role-equivalence table for ambient counterexamplehood}, colback=cyan!1!white, colframe=cyan!45!black]
\small
The equivalence asserted above is role-equivalence, not terminological identity. Ordinary proof systems need not use AASC labels; they instantiate the kernel when they perform the corresponding work.
\begin{center}
\scriptsize
\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{0.98\textwidth}{@{}L{0.34\textwidth}L{0.18\textwidth}Y@{}}
\toprule
\textbf{Ambient proof/refutation role} & \textbf{Kernel role} & \textbf{Why the role is required for counterexamplehood}\\
\midrule
Admissible object formation in the official domain & Adm & Without an admitted object in the theorem's domain, there is no object that can instantiate the negation.\\
Proof-standing as a counterexample rather than informal assertion & St & Without standing, the object carries no theorem-bearing consequence.\\
Fixed reference to the same universal theorem and hypotheses & Ref & Without reference preservation, the alleged refutation may concern a different theorem.\\
Final contradiction of the asserted conclusion & Irr & Without finality, the alleged refutation can be repaired, deferred, or retyped after the act.\\
No target drift, hidden selector, or scope transfer & Same-regime fidelity & Without same-regime fidelity, the alleged counterexample no longer refutes the official statement.\\
\bottomrule
\end{tabularx}
\end{center}
Thus an ambient proof system either performs the kernel roles or fails to supply theorem-bearing counterexamplehood. This is a governance-equivalence claim, not a request to replace ordinary mathematical syntax with AASC vocabulary.
\end{tcolorbox}

\begin{manuallemma}[3.K4 (Counterexamplehood is theorem-bearing construction)]
\phantomsection\manualcurrentlabel{3.K4}\label{lem:counterexamplehood-theorem-bearing-construction}
A counterexample to a fixed universal mathematical theorem is not merely an object-level expression satisfying the syntactic negation. It is a theorem-bearing construction: an admissible object or tuple must be identified, verified to satisfy the theorem's hypotheses, shown to contradict the asserted conclusion, and kept faithful to the same target theorem.
\end{manuallemma}

\begin{proof}
A universal theorem has the form ``for every object in a fixed class, the asserted conclusion holds.'' A counterexample therefore must identify an object in that fixed class and establish that the conclusion fails for that same object under the same hypotheses. If the object is not admissibly in the fixed class, it is not a counterexample. If the verification has no proof-standing, it does not refute the theorem. If reference to the theorem changes, the target has changed. If the contradiction is not final for the theorem-bearing act, it is not a completed refutation. Hence counterexamplehood is an admissibility-assessable, identity-preserving construction inside the ambient proof/refutation regime.
\end{proof}

\begin{manualtheorem}[3.K5 (Fefferman counterexample subsumption)]
\phantomsection\manualcurrentlabel{3.K5}\label{thm:fefferman-counterexample-subsumption}
Any ambient ZFC / classical-analysis counterexample to Fefferman's periodic Navier--Stokes statement~(B), if it is to refute the official theorem, is an admissibility-assessable, identity-preserving construction on the fixed official periodic domain. It therefore instantiates the fixed-scope kernel roles, regardless of whether the proof is written in AASC terminology.
\end{manualtheorem}

\begin{proof}
Such a counterexample would have to exhibit an official datum $u_0\in C^\infty(\T^3;\R^3)$ with $\nabla\cdot u_0=0$, identify the corresponding maximal classical solution pair $(u,p)$ on the fixed torus carrier with viscosity $\nu>0$ and the fixed pressure-gauge convention, establish finite maximal time $T_*(u_0)<\infty$, establish failure of same-scope smooth continuation past that horizon, and show that this tuple contradicts the universal statement of Fefferman~(B). Each step is theorem-bearing: the datum must be admissible for the official class, the solution must preserve reference to the official equation and carrier, the non-continuation claim must be same-target, and the contradiction must have final consequence for the theorem. By Lemma~\ref{lem:counterexamplehood-theorem-bearing-construction} and Theorem~\ref{thm:ambient-proof-system-subsumption}, this is a kernel-instantiating proof/refutation construction. The kernel is therefore not an optional overlay on the counterexample; it is the role structure required for the alleged object to refute the theorem at all.
\end{proof}

\begin{manualcorollary}[3.K6 (No ambient proof-system escape)]
\phantomsection\manualcurrentlabel{3.K6}\label{cor:no-ambient-proof-system-escape}
Appealing to an ambient proof system such as ZFC or classical analysis does not exempt a proposed counterexample from the fixed-scope kernel. Either the ambient system supplies admissibility, standing, reference, same-target fidelity, and final consequence, in which case the kernel roles are instantiated by governance-equivalence; or it does not, in which case the alleged object cannot function as a theorem-bearing counterexample.
\end{manualcorollary}

\begin{proof}
Immediate from Theorem~\ref{thm:ambient-proof-system-subsumption} and Theorem~\ref{thm:fefferman-counterexample-subsumption}. The issue is not vocabulary but role performance. An ambient proof system that can refute a fixed theorem must perform the kernel roles; an ambient assertion that performs none of those roles has no theorem-bearing standing.
\end{proof}
...
\noindent\textbf{Source provenance and manuscript role.} The source provenance of P1 is recorded in Appendix~\ref{sec:internal-ledger}, but the theorem is proved in this manuscript and no external source is used load-bearingly here. In the present manuscript P1 is used only to separate heuristic proposal-generation, exploratory PDE intuition, and auxiliary analytic guessing from standing-bearing same-scope admissibility claims. It therefore blocks the reading on which Part~II is expected to furnish a conventional PDE continuation remedy before the structural burden has been discharged.

\begin{remark}[Manuscript-local application of P1]\label{rem:ametric-application-pde-misread}
P1 is jurisdictional, not analytic. Exploratory continuation guesses, heuristic singularity scenarios, and informal PDE-side proposal generation may suggest candidate pictures, but they do not carry same-scope licensing authority for the official periodic theorem. In this manuscript they therefore cannot count as missing admissibility ingredients, hidden continuation gates, or latent PDE-side remedies waiting to be discovered later in Part~II or Part~III.
\end{remark}
...
&t(h(x))=t(h(y))\\
&\text{for every finite same-scope history }h
\text{ and every admitted test }t.
\end{aligned}
\]
...
periodic carrier, the live route verifies this label by the carrier-certified packet of
Theorem~\ref{thm:ns-carrier-verifies-rule-package} and
Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}, so the official endpoint theorem does not
rely on it as an unproved carrier-specific primitive. The later continuation-mechanism reformulation of
Section~13.2B is retained as explanatory architecture; it is not required as an additional live source
of Structural Exhaustion on the official theorem route.
\end{remark}
...
Equivalently: the vortex-stretching wall marks the incompleteness of same-scope PDE analytic closure; it does not reopen primitive admissibility, does not create a second same-scope interior, and does not license a same-scope gate.
...
If the admissibility map is complete and non-extendable, then no further conventional same-scope supplement may be
...
same tensor compatibility. If the decomposition changes admissible consequences, it is not a
...
bookkeeping distinctions for the fixed torus carrier.  Nothing in this subsection adds a same-scope
...
and standard frequency decomposition), and passage among them does not create a continuation
...
The statements below are proved \emph{in this manuscript}. They do not add a new PDE estimate, blow-up criterion, or same-scope continuation remedy. Their task is to prove that the fixed periodic problem is a non-degenerate standing-bearing fixed scope and therefore falls under the fixed-scope kernel by necessity, via Theorem~\ref{thm:kernel-necessity-nondegenerate-regimes} and Corollary~\ref{cor:kernel-applies-official-periodic-necessity}. The subsequent bridge then applies the structural theorem packet \StructuralPacketRef{}, with P1--P4 proved in-paper and the minimal P4 gate/exhaustion core supplied in Appendix~A. Every later structural appeal in the bridge, carrier-identification, and transport packets is therefore routed through manuscript-local theorem numbers rather than companion papers. In particular, Lemma~\ref{lem:bridge-periodic-boundary-closure} is only the scope-fixity / typing component of the bridge packet; the no-second-gate consequence is carried later by Theorem~\ref{thm:bridge-periodic-applicability}. This is the place where the manuscript answers, in the theorem-local chain, the recurrent misreading that keeps searching for a missing PDE-side continuation ingredient.
\end{tcolorbox}


\begin{manualtheorem}[12A.0 (Official periodic Navier--Stokes proof/refutation regime is non-degenerate)]
\phantomsection\manualcurrentlabel{12A.0}\label{thm:official-periodic-ns-nondegenerate}
The proof/refutation regime induced by Fefferman's official periodic Navier--Stokes problem on $\T^3$---with $\nu>0$, zero forcing, smooth divergence-free periodic data, the fixed mean-zero pressure gauge, the standard lawful redescription class, and the classical smooth continuation question---is a non-degenerate theorem-bearing regime in the sense of Definition~\ref{def:nondegenerate-theorem-bearing-regime}. The claim is not that the differential equations themselves are a construction act; it is that proving or refuting Fefferman~(B) requires an admissibility-assessable, identity-preserving theorem-bearing regime on the fixed official domain.
\end{manualtheorem}


\begin{proof}
We verify the clauses of Definition~\ref{def:nondegenerate-theorem-bearing-regime} explicitly.
\begin{center}
\footnotesize
\renewcommand{\arraystretch}{1.12}
...
\toprule
\textbf{Non-degeneracy clause} & \textbf{Official periodic Navier--Stokes witness}\\
\midrule
\textbf{Determinate reference} & The carrier is the fixed torus $\T^3$, with fixed viscosity $\nu>0$, zero forcing, the incompressible Navier--Stokes equations, the smooth periodic divergence-free datum class, and the fixed pressure-gauge convention.\\
\textbf{Fixed comparison domain} & The domain is exactly the official periodic comparison class of Definition~\ref{def:fixed-domain-comparison-class}: same torus, same viscosity, same forcing status, same datum class, same gauge, same lawful redescription class, and same continuation question.\\
\textbf{Standing-bearing consequence} & Fefferman~(B) is a truth-valued universal theorem over official periodic data: every datum either has the asserted global smooth realization or a same-scope counterexample must supply a finite-horizon endpoint condition.\\
\textbf{Lawful redescription identity} & Velocity, vorticity, pressure, strain, and frequency presentations are deterministic lawful skins of the same smooth periodic state by Proposition~\ref{prop:adequacy-lawful-redescription} and Lemma~\ref{prop:equiv-completion-NS}.\\
\textbf{Irreversible commitment} & Once the official theorem is fixed, changing carrier, datum class, gauge, equation, lawful-skin class, or continuation question changes the problem instead of repairing an endpoint claim.\\
\textbf{Fail-closed admissibility} & A proposal outside the fixed-domain comparison class is rejected as a different-domain proposal, not retained as a third same-scope counterexample status.\\
\textbf{Nontrivial admitted content} & The zero datum supplies a standing-positive same-scope carrier state, and the local periodic realization packet of Appendix~B supplies ordinary local classical realization for smooth admitted data.\\
\textbf{No post-hoc repair} & A failed same-scope endpoint cannot be repaired by later selectors, tolerances, gauge shifts, presentation switches, or altered continuation questions without leaving the declared problem.\\
\bottomrule
\end{tabularx}
\end{center}
Each witness is part of the official periodic proof/refutation setup rather than an added regularity assumption. The equations are the object-level dynamics; the theorem-bearing act of proving or refuting Fefferman~(B) is the regime being classified. Thus the official periodic Navier--Stokes proof/refutation regime is non-degenerate in the precise fixed-scope sense needed for the kernel; denying this theorem requires denying that an alleged proof or counterexample to Fefferman~(B) must preserve fixed reference, fixed domain, lawful presentation identity, admissible object formation, and stable theorem-bearing consequence.
\end{proof}

\begin{manualcorollary}[12A.0.1 (Kernel applies to the official periodic problem by necessity)]
\phantomsection\manualcurrentlabel{12A.0.1}\label{cor:kernel-applies-official-periodic-necessity}
By Theorem~\ref{thm:kernel-necessity-nondegenerate-regimes}, Theorem~\ref{thm:fefferman-counterexample-subsumption}, and Theorem~\ref{thm:official-periodic-ns-nondegenerate}, the fixed-scope kernel applies to the official periodic Navier--Stokes proof/refutation regime by necessity, not by optional adoption of a framework.
\end{manualcorollary}

\begin{proof}
Theorem~\ref{thm:official-periodic-ns-nondegenerate} proves that the proof/refutation regime induced by the official periodic problem is a non-degenerate theorem-bearing regime. Theorem~\ref{thm:fefferman-counterexample-subsumption} shows that any ambient counterexample to Fefferman~(B) is an admissibility-assessable construction on that fixed official domain. Theorem~\ref{thm:kernel-necessity-nondegenerate-regimes} says that every such non-degenerate regime instantiates the kernel. Therefore the official periodic proof/refutation regime instantiates the kernel by necessity.
\end{proof}
...
\noindent\emph{Case 2.} If the proposed criterion leaves admissibility outcomes unchanged, then it is continuation-neutral bookkeeping or skin in the sense of Proposition~\ref{prop:adequacy-role-triad}; it adds no same-scope admissibility authority.
...
The next three statements are the minimal fixed-periodic standing/intelligibility packet later used to control the endgame. To avoid collision with the branch labels $\{S1,S2,S3,S4,S5\}$ reserved later for the constructive branch-forcing set, this packet is labeled \emph{ST1--ST5}. The present subsection carries the fixed-scope half of that packet. No companion paper is needed to recover these clauses from the submitted PDF.
...
same-scope continuation gate on the fixed periodic carrier.
\end{corollary}
...
supplies a genuinely same-scope continuation gate.
\end{proof}
...
discriminator, it would define a same-scope gate or a parameterized family of such gates on the
...
carrier, the repair would amount to a same-scope continuation gate or generator, contradicting
...
The finite standing-positive decision packet compresses the Part~II forcing step into a finite decision packet before zero-parameter certification is stated. Once the bridge packet and the minimal rule package are in the theorem-local chain, every standing-positive continuation-relevant candidate determination on the fixed carrier is tested in exactly this order:
\begin{enumerate}[leftmargin=2.2em]
...
\item \emph{scope change}: it alters one of the fixed carrier commitments and is therefore not a same-scope candidate.
\end{enumerate}
\end{manualdefinition}
...
\begin{manualproposition}[13.5G (Existing carrier content adds no continuation gate)]
...
continuation authority; it simply names content already present in the carrier.
\end{proof}
...
candidate that changes one of those commitments is not operating inside the fixed-scope problem.
...
so they add no gate by Corollary~\ref{cor:13-5L-rival-elimination}. Geometric-structure gates are
...
\paragraph{Why no gates can appear.}
...
use would therefore constitute a same-scope operator family, ruled out by the specialized periodic
...
table is not a theorem but an explicit reading guide to the carrier-representative content
...
\item $Q$ does not itself define a same-scope continuation gate, admissibility selector, or competing discriminator.
\end{enumerate}
...
\addlinespace
realized velocity-pressure pair $(u,p)$ in the fixed gauge & lawful skin presentation & presents one realized lineage without creating a second gate or a continuation criterion & Definition~\ref{def:standard-carrier}; Definition~\ref{def:realized-lineage-same-object-convention} \\
\addlinespace
...
\item $\mathcal M$ is quotient-visible witness bookkeeping subordinate to the already fixed pre-horizon witness, horizon discriminator, or singular-candidate extractor, and therefore adds no same-scope gate;
...
\begin{manualdefinition}[13.23A.M (Three roles of metric quantities)]
\phantomsection\manualcurrentlabel{13.23A.M}\label{def:metric-quantity-three-roles}
On the fixed periodic carrier, a metric quantity may appear in three distinct roles. First, it may be an \emph{analytic diagnostic}: valid PDE bookkeeping such as an $H^m$ norm, a BKM integral, a critical norm, a profile descriptor, or a vortex-stretching observable. Second, it may be a \emph{continuation-obstruction witness}: a pre-horizon record of why the standard local continuation radius is not being asserted uniformly. Third, it may be promoted to \emph{same-scope endpoint authority}: a purported independent rule deciding the official endpoint status. The first two roles are legitimate bookkeeping / witness roles; the third is forbidden unless it factors through the already fixed finite-horizon endpoint condition and horizon discriminator.
\end{manualdefinition}


\begin{manualtheorem}[13.23A.6 (Metric Diagnostic Non-Authority Theorem for the periodic carrier)]
\phantomsection\manualcurrentlabel{13.23A.6}\label{thm:metric-non-authority-periodic-carrier}
Let $\mathcal M$ be any norm divergence, BKM-type observable, scale-critical threshold, profile concentration descriptor, or $\Theta_x$-derived growth statement formed from lawful periodic Navier--Stokes data on the fixed carrier. Such a quantity remains a valid analytic diagnostic and may serve as a continuation-obstruction witness in the sense of Definition~\ref{def:metric-quantity-three-roles}. What it cannot do, merely because it is metric, large, divergent, or classically familiar, is become an independent same-scope endpoint authority. On the live same-scope route it has exactly one of the following endpoint-status roles:
\begin{enumerate}[label=(\roman*), leftmargin=2.2em]
\item licensed metric bookkeeping with no endpoint authority;
\item quotient-visible witness bookkeeping subordinate to Lemma~\ref{lem:finite-horizon-witness}, \HorizonDetectabilityRef{}, Definition~\ref{def:finite-horizon-endpoint-condition}, or Definition~\ref{def:singular-candidate-extractor}; or
\item an illicit metric gate / scope change if promoted as an independent continuation discriminator.
\end{enumerate}
In particular, metric divergence may record the pre-horizon visibility of a purported failure, but it cannot create a same-scope endpoint status.
\end{manualtheorem}

\begin{proof}
Proposition~\ref{prop:metric-divergence-skin-non-authority} proves that lawfully formed metric quantities on the fixed periodic carrier are licensed interior bookkeeping and have no independent standing authority. Proposition~\ref{prop:quotient-visible-witness-vs-illicit-metric-gate} proves the complete same-scope trichotomy for metric divergence claims: bookkeeping, quotient-visible witness bookkeeping, or illicit extra gate / scope change. Proposition~\ref{prop:boundary-licensing-same-scope-compatibility} records the compatibility point needed here: non-generative metric licensing does not supply a coercive estimate, while same-scope admissibility still governs the licit status of singularity representations. Definition~\ref{def:metric-quantity-three-roles} separates this non-authority claim from any denial of ordinary PDE diagnostic value. The listed observables are all metric descriptors or metric-derived descriptors; therefore they fall under that trichotomy and have no fourth same-scope endpoint-authority role.
\end{proof}
...
This subsection is now downstream of the finite decision packet 13.5E--13.5L and of the carrier-certified
Structural Exhaustion packet of Theorem~\ref{thm:ns-carrier-verifies-rule-package} and
Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}. Its role is not to be the first
place where finite same-scope exhaustion appears, and on the narrow horizon-detectability route used
for the official periodic theorem it is not a separate live node. Rather, it repackages the earlier
typed classification in the continuation-mechanism language used for translation and local bookkeeping
in the later branch-forcing and terminality discussion. In that local language, the only surviving
same-scope continuation mechanisms are: lawful-skin bookkeeping, horizon factorization through $D_T$,
or bundle deformation forcing one of $(S1)$--$(S5)$. Thus 13.2B is a carrier-local
continuation reformulation of already-certified standing-positive same-scope closure, not as a fresh
proof hinge.
...
\begin{manualcorollary}[13.23H (Architectural continuation-mechanism reformulation of the standing-positive carrier route)]
\phantomsection\manualcurrentlabel{13.23H}\label{cor:derived-structural-exhaustion}
For the standard periodic Navier--Stokes carrier, the bookkeeping / horizon / deformation trichotomy of
Theorem~\ref{thm:carrier-exhaustive-trichotomy} is an architectural continuation-mechanism reformulation
of the carrier-certified Structural Exhaustion packet already verified in
Theorem~\ref{thm:ns-carrier-verifies-rule-package} and
Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}. On the narrow
horizon-detectability route used for the official periodic theorem, later load-bearing steps cite those
carrier-certified labels together with the finite packet 13.5E--13.5L and
Proposition~\ref{prop:no-additional-pde-side-criterion} rather than this corollary.
\end{manualcorollary}

\begin{proof}
Theorem~\ref{thm:carrier-exhaustive-trichotomy} reformulates the already-certified standing-positive
same-scope closure of the standard periodic carrier in continuation-mechanism language: bookkeeping,
horizon factorization through $D_T$, or deformation forcing one of $(S1)$--$(S5)$, with no fourth
standing-positive same-scope continuation-relevant kind. This is the same carrier-side no-residue
content already verified on the live route by Theorem~\ref{thm:ns-carrier-verifies-rule-package} and
Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}; the present corollary records that
equivalence for architectural translation only.
\end{proof}
...
The continuation-mechanism reformulation of Section~13.2B, read together with the carrier-certified
Structural Exhaustion packet of Theorem~\ref{thm:ns-carrier-verifies-rule-package} and
Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}, has a fixed-scope existence
meaning for the standard periodic carrier. The present subsection records that meaning explicitly. It
is interpretive rather than load-bearing: it explains what the already-certified standing-positive
same-scope closure says about existence on the fixed carrier, without replacing the proof work carried
by Part~II and Part~III.
...
paper; equivalently, fix the comparison class of Definition~\ref{def:fixed-domain-comparison-class}. Then the standard periodic Navier--Stokes carrier is not merely a disciplined formulation: it
...
\end{enumerate}
Thus neither a same-scope continuation gate nor a hidden PDE-side discriminator survives once the
...
additional same-scope proof power. This is the explicit Part~II answer to the analytic-reader expectation
...
\begin{manualproposition}[13K.1 (Fixed-domain counterexample burden)]
\phantomsection\manualcurrentlabel{13K.1}\label{prop:fixed-domain-counterexample-burden}
Every proposed finite-horizon singular counterexample to the official periodic theorem has exactly one of the following statuses relative to the fixed-domain comparison class of Definition~\ref{def:fixed-domain-comparison-class}:
\begin{enumerate}[label=(\roman*), leftmargin=2.2em]
\item it preserves the fixed-domain comparison class, in which case it is evaluated by the same-scope exhaustion, no-new-criterion, realized-lineage status-trichotomy, and branch-elimination chain already fixed in this manuscript; or
\item it alters at least one fixed-domain commitment, in which case it is not a counterexample to the fixed periodic theorem stated in Theorem~\ref{thm:official-periodic-pde-theorem}.
\end{enumerate}
Thus a skeptical proposal must exhibit a standing-bearing difference while preserving the declared comparison class; merely supplying a richer analytic regime, a different weak formulation, a new profile language, or a different metric descriptor does not by itself engage the theorem.
\end{manualproposition}

\begin{proof}
Let a proposed finite-horizon singular endpoint condition be given. If it changes the torus, viscosity, forcing status, smooth periodic datum class, pressure gauge, lawful-redescription class, or continuation question, then by Definition~\ref{def:fixed-domain-comparison-class} it has left the comparison class and is not a same-scope counterexample. Assume instead that all fixed-domain commitments are preserved. Then Definition~\ref{def:same-scope-counterexample-fixed-domain} makes the finite-horizon condition a same-scope endpoint condition on the already identified carrier. Proposition~\ref{prop:no-additional-pde-side-criterion} rules out additional PDE-side continuation criteria, Theorem~\ref{thm:bridge-periodic-applicability} supplies the fixed-scope exhaustion consequence, and Part~III later transports the same burden to the realized-lineage status trichotomy and branch-elimination packet. Hence every genuine same-scope counterexample is already inside the local exhaustion chain, while every alleged endpoint outside that chain has changed domain.
\end{proof}

\begin{remark}[Relative uniqueness under locked scope]\label{rem:relative-uniqueness-locked-scope}
The words ``unique'', ``forced'', and ``exhausted'' in the carrier-identification packet are relative to the fixed-domain comparison class. They do not assert uniqueness across unconstrained mathematical imagination; they assert that, once the official periodic carrier and continuation question are locked, no standing-preserving same-scope alternative remains.
\end{remark}
...
After the carrier-identification packet 13I--13L, the standard carrier is not presented as one
disciplined formalization among several. A reader who wants to resist the later singularity
exclusion must reject either the fixed-scope primitive necessity chain, the proof that the
...
The decisive contradiction now comes before the branch taxonomy. Once the standard periodic carrier has been identified with the \emph{unique admissible interior} by Theorem~\ref{thm:periodic-carrier-unique-interior}, a same-scope singularity cannot be imagined as living in some unmentioned admissible regime. It would have to arise either from a second same-scope interior, from an admissible variation or extra continuation discriminator inside the unique interior, or from a scope change misdescribed as same-scope continuation. The later quotient and branch machinery is diagnostic only: it records how such impossible escape attempts would present themselves concretely on the fixed carrier.
...
commitments.
Then every standing-positive same-scope continuation-relevant structure admitted on this route is already
interior to the standard carrier or a lawful same-scope functional thereof by
Theorem~\ref{thm:scope-completeness-standard-carrier}. The finite decision packet 13.5E--13.5L
therefore leaves only the typed possibilities already isolated on the fixed carrier: existing carrier
content, bookkeeping-only variation, parameterized gate, competing discriminator, or scope change.
Existing carrier content and bookkeeping-only variation add no continuation authority by
Propositions~\ref{prop:13-5G-existing-carrier-content-adds-no-gate} and
\ref{prop:13-5H-bookkeeping-no-standing-force}; parameterized and competing same-scope gates are
excluded by Propositions~\ref{prop:13-5I-parameterized-gates-forbidden} and
\ref{prop:13-5J-competing-discriminator-forbidden}; and scope changes are not same-scope rivals by
Proposition~\ref{prop:13-5K-scope-change-not-rival}. The carrier-certified Structural Exhaustion packet
of Theorem~\ref{thm:ns-carrier-verifies-rule-package},
Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}, and
Proposition~\ref{prop:no-additional-pde-side-criterion} then removes any residual attempt to reinstall
a further same-scope continuation selector or PDE-side criterion on that standing-positive route.
Thus, to keep the asserted event inside the same lineage while changing continuation status, one is
forced into a deformation of the fixed carrier-side commitments rather than a new licit same-scope
criterion. Concretely, one must therefore do at least one of the following: weaken a bundle clause,
isolate one interior compatibility from the rest, insert a tolerance or selector, privilege a
non-equivalent redescription, or justify an earlier failure only by downstream composition. These are
exactly (S1)--(S5), typed via envelope licensing, lawful redescription, and composition discipline
(Axioms~\ref{ax:envelopes}, ~\ref{ax:redescription},~\ref{ax:composition}, and
\ref{ax:impossibility}). In this way the forcing step rests on the carrier-certified exhaustion packet
rather than on Rule label~\ref{ax:exhaustion} as an external primitive.
\end{proof}
...
\addlinespace
Quotient-visible branch & No gate is introduced; the singularity mark is already a visible incompatibility of the admitted interior. & Collapses by saturation, fail-closed enforcement, and the impossibility of repair in-lineage. \\
\bottomrule
\end{tabular}
...
The proof order in the endgame is explicit:
\[
\begin{aligned}
...
quotient and therefore contributes no same-scope discriminator; the mark ``singular'' is a
...
Part II, by itself, does \emph{not} claim a PDE-native continuation-estimate proof as its method. In particular, it does not furnish:
\begin{enumerate}[label=(\alph*), leftmargin=2.2em]
...
\textbf{Step 2 (Exhaustion constraint).} Structural Exhaustion forbids introducing a same-scope
...
reinterpretations, or local repair.

\textbf{Step 5 (Conclusion).} The result is therefore correctly typed as a primitive-side structural resolution inside the admissibility map: Part~II carries the carrier-side certification burden and excludes same-scope singular existence there, rather than supplying a conventional PDE-native continuation-estimate proof.
\end{proof}
...
\item repair of a rejected lineage (post-hoc repair within a failed sequence);
...
carrier-certified Structural Exhaustion packet of Theorem~\ref{thm:ns-carrier-verifies-rule-package}
and Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier}, together with
Proposition~\ref{prop:no-additional-pde-side-criterion}, and is rejected at entry by fail-closed
enforcement (Axioms~\ref{ax:failclosed}, \ref{ax:enforcement}).
...
Consequently, no genuinely same-scope analytic route can overturn the certified periodic result.
...
Because the carrier commitments are held fixed, the finite typed packet 13.5E--13.5L applies to any
standing-positive same-scope continuation-relevant candidate determination it introduces. Hence any
such technique either reduces to existing carrier content, reduces to bookkeeping-only variation, is
forbidden as a parameterized or competing same-scope gate, or changes scope by
Theorem~\ref{thm:13-5F-finite-same-scope-classification} through
Corollary~\ref{cor:13-5L-rival-elimination}. Therefore the technique is not an external enlargement of
the continuation picture but a move already covered by the certified same-scope continuation space of
the standard carrier.

By Corollary~\ref{cor:structural-exhaustion-verified-standard-carrier},
Proposition~\ref{prop:no-additional-pde-side-criterion}, and Theorem~\ref{thm:terminality}, no
same-scope continuation move remains available that could reopen the regularity question on the carrier
already identified with the unique admissible interior. Therefore any purported analytic reopening that
still changes the outcome must alter the carrier, the continuation notion, or the lawful skin
structure; that is, it is a scope change rather than a same-scope analytic escape. Such scope changes
do not alter the certified official theorem on the original carrier.
\end{proof}
...
\item each proposed conventional same-scope supplement corresponds to a prohibited move in the map;
...
\item the map is exhausted, so no same-scope route may be added;
