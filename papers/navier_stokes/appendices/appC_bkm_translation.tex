...
The structural point is not that $\mathcal I_x$ fails to exist, nor that the BKM criterion is invalid as PDE analysis. The point is role discipline: using $\mathcal I_x$ as an independent same-scope endpoint authority requires an extra threshold, tolerance, or horizon selector not contained in the fixed carrier.
...
\begin{manualproposition}[C.3A (BKM observable as metric witness bookkeeping, not endpoint authority)]
\phantomsection\manualcurrentlabel{C.3A}\label{prop:bkm-metric-witness-not-authority}
The BKM-type observable $\mathcal I_x$ has two different roles that must not be conflated. As a classical conditional continuation observable it can record a standard pre-horizon blow-up implication and may witness obstruction to a local continuation estimate. As a same-scope endpoint authority, however, it has no independent force: on the fixed periodic carrier it is either licensed metric bookkeeping, quotient-visible witness bookkeeping subordinate to the horizon discriminator, or an illicit metric gate if promoted beyond that role.
\end{manualproposition}

\begin{proof}
The observable $\mathcal I_x$ is computed from the realized vorticity field and therefore is a lawful metric descriptor on the realized lineage. Lemma~\ref{lem:bkm-growth-at-breakdown} records the standard conditional continuation consequence. But Theorem~\ref{thm:metric-non-authority-periodic-carrier} applies to BKM-type observables specifically: metric divergence does not create a same-scope endpoint status. Thus $\mathcal I_x$ may serve as witness bookkeeping for an already fixed finite-horizon failure, but any attempt to use it as an independent same-scope gate is illicit.
\end{proof}
...
PDE-side metric quantity. Proposition~\ref{prop:bkm-metric-witness-not-authority} fixes its licit status: it may be bookkeeping or witness bookkeeping, but not independent endpoint authority. The extra structure enters when one attempts to turn $\mathcal I_x$ into a
...
instances of large vorticity matter, then the rule not treats the bundle clauses uniformly on the
